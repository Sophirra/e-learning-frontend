...
export function CreateQuizPage() {
    ...
    const checkQuestionReady = (question: Question): boolean => {
        return !!(
            ...
            question.answers.filter(a => a.isCorrect).length >= quiz.correctAnswersCount
        );
    };
    ...
    const saveCurrentQuestion = () => {
        ...
        if (updatedQuestions[currentQuestionIndex]) {
            updatedQuestions[currentQuestionIndex].isReady = checkQuestionReady(updatedQuestions[currentQuestionIndex]);
            setQuiz({
                ...quiz,
    ...
    const handleToggleCorrect = (answerId: string) => {
        ...
        if (quiz.isMultipleChoice) {
            currentQuestion.answers[answerIndex].isCorrect = !currentQuestion.answers[answerIndex].isCorrect;
        } else {
                ...
                <div className="mt-6 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <Toggle
                            ...
                            onPressedChange={(value) => setQuiz({...quiz, isMultipleChoice: value})}
                        >
                            Multiple Choice
                        </Toggle>
                        <Select
                            value={quiz.correctAnswersCount.toString()}
                            onValueChange={(value) => setQuiz({...quiz, correctAnswersCount: parseInt(value)})}
                        >
                            <SelectTrigger className="w-[180px]">
                        ...
                        <ScrollArea className="h-[calc(100vh-300px)]">
                            {quiz.questions.map((question, index) => (
                                <Tooltip key={question.id}
                                         content={!question.isReady ? "Question is not ready" : undefined}>
                                    <Button
                                        variant="ghost"
                                        className={cn(
                                ...
                                <div className="space-y-4">
                                    ...
                                    <textarea
                                        className={cn(
                                            ...
                                            !quiz.questions[currentQuestionIndex].description && "border-red-500"
                                        )}
                                        value={quiz.questions[currentQuestionIndex].description}
                                        onChange={(e) => {
                                            const updatedQuestions = [...quiz.questions];
                                            updatedQuestions[currentQuestionIndex].description = e.target.value;
                                            setQuiz({...quiz, questions: updatedQuestions});
                                        }}
                                    ...
                                    {quiz.questions[currentQuestionIndex].answers.map((answer) => (
                                        <div key={answer.id} className="flex items-center gap-2">
                                            ...
                                            <Input
                                                ...
                                                onChange={(e) => {
                                                    const updatedQuestions = [...quiz.questions];
                                                    const answerIndex = updatedQuestions[currentQuestionIndex].answers
                                                        .findIndex(a => a.id === answer.id);
                                                    updatedQuestions[currentQuestionIndex].answers[answerIndex].text = e.target.value;
                                                    setQuiz({...quiz, questions: updatedQuestions});
                                                }}
                                                className="flex-1"
                                            />
