...
export function StudentCalendar() {
            ...
            .then((res) => {
                ...
                const mappedClasses: ClassTileProps[] = data
                    .map((cls) => {
                        ...
                        else if (start <= now && now.getTime() - start.getTime() < 60 * 60 * 1000)
                            // 60 * 60 * 1000 = 1 hour
                            state = "ongoing";
                ...
                setSelectedClassId((prev) => (prev && !data.some((c) => c.id === prev) ? null : prev));
            })
            .catch((err) => {
    ...
    useEffect(() => {
        ...
        const source = selectedClassId ? timeline.filter((c) => c.id === selectedClassId) : timeline;
