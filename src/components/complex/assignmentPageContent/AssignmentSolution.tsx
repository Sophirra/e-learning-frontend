...
export function AssignmentSolution({
    ...
    const solutionFiles = assignment.files?.filter(file => file.type === 'solution');
        ...
        >
            <div className="flex flex-col gap-2 p-2 text-xs font-normal ml-1">
                {solutionFiles?.length === 0 || solutionFiles === undefined ? (
                    <Label className="mt-2 ml-2 font-light">No attached files found for this assignment.</Label>
                ) : (
                    solutionFiles?.map((file) => (
                        ...
                        >
                            <a href={file.path} target="_blank" rel="noopener noreferrer">
                                <Button variant="link" className="pl-0">{file.name}</Button>
                            </a>
                            <Label className="font-light">
                                uploaded {file?.uploadDate && new Date(file.uploadDate).toLocaleDateString('pl-PL', {
                                day: '2-digit',
                                month: '2-digit',
                            })}
                            </Label>
